...
class AllKOTwiseReportPage extends StatefulWidget {
  ...
  @override
  State<AllKOTwiseReportPage> createState() => _AllKOTwiseReportPageState();
}

class _AllKOTwiseReportPageState extends State<AllKOTwiseReportPage> {
    ...
    if (selectedDbKey == null || selectedDbKey == "All") {
      for (final list in dbToKOTAnalysis.values) {
        for (final report in list) {
          _allRows.addAll(_KOTwiseRow.fromReport(report: report, restaurant: "ALL"));
        }
      }
    } else {
      for (final list in dbToKOTAnalysis.values) {
  ...
  Future<void> _exportExcel() async {
    ...
    final String path = '${Directory.current.path}/KOTAnalysisReport.xlsx';
                ...
                builder: (context, constraints) {
                  return Column(
                    ...
                    children: [
                      ...
                      // ✅ Restaurant dropdown + styled Search button
                      Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          _dropdownFilter(
                          ...
                          Padding(
                            padding: const EdgeInsets.only(top: 20), // ✅ aligned properly
                    ...
                    itemBuilder: (context) => _allColumns.map((col) {
                      return CheckedPopupMenuItem<_Col>(
                        ...
                        child: Text(
                          col.title,
                          style: TextStyle(
                            color: col.title == 'KOT ID' ? Colors.red[700] : Colors.black,
                            fontWeight: col.title == 'KOT ID' ? FontWeight.bold : FontWeight.normal,
                          ),
                        ),
                      );
                    }).toList(),
      ...
      children: List.generate(_allRows.length, (i) {
        ...
        return Container(
          ...
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: _visibleColumns.map((col) {
              final isProduct = col.key == 'product';
...
class _KOTwiseRow {
  ...
  // Each KOT can produce multiple display rows (one per item)
