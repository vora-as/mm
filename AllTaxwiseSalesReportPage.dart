...
class _AllTaxwiseSalesReportPageState extends State<AllTaxwiseSalesReportPage> {
  ...
  final List<_Col> _allColumns = [
    ...
    _Col('Tax Name', 'taxName'),
    _Col('Tax Percent', 'taxPercent'),
    _Col('Tax Amount', 'taxAmount'),
  ];
  late List<_Col> _visibleColumns;

  List<_TaxwiseSalesRow> _allRows = [];
  ...
  Future<void> _fetchData() async {
    ...
    if (selectedDbKey == null || selectedDbKey == "All") {
      for (final list in dbToTaxwise.values) {
        for (final report in list) {
          _allRows.add(_TaxwiseSalesRow.fromReport(
            report: report,
            restaurant: "ALL",
          ));
        }
  ...
  _TaxwiseSalesRow get totalRow {
    double sum(String Function(_TaxwiseSalesRow) getter) =>
        _allRows.fold(0.0, (a, b) => a + double.tryParse(getter(b))!);

    return _TaxwiseSalesRow(
      restaurant: "Total",
      billNo: "",
      billDate: "",
      taxName: "",
      taxPercent: "",
      taxAmount: sum((r) => r.taxAmount).toStringAsFixed(2),
    );
  }
  ...
  Future<void> _exportExcel() async {
    ...
    reportCell.value = "All Taxwise Sales Report";
    ...
    final String path = '${Directory.current.path}/AllTaxwiseSalesReport.xlsx';
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Padding(
                            padding: const EdgeInsets.only(top: 20), // ✅ consistent vertical alignment
                    ...
                    itemBuilder: (context) => _allColumns.map((col) {
                      return CheckedPopupMenuItem<_Col>(
                        ...
                        child: Text(
                          col.title,
                          style: TextStyle(
                            color: col.title == 'Tax Name' ? Colors.red[700] : Colors.black,
                            fontWeight: col.title == 'Tax Name' ? FontWeight.bold : FontWeight.normal,
                          ),
                        ),
                      );
                    }).toList(),
...
class _TaxwiseSalesRow {
  ...
  factory _TaxwiseSalesRow.fromReport({required TaxwiseReport report, required String restaurant}) {
    return _TaxwiseSalesRow(
      restaurant: restaurant,
      billNo: report.billNo,
      billDate: report.billDate,
      taxName: report.taxName,
      taxPercent: report.taxPercent,
  ...
  dynamic getField(String key) {
    switch (key) {
      ...
      case 'taxName': return taxName ?? "";
      case 'taxPercent': return taxPercent ?? "0.00";
      case 'taxAmount': return taxAmount ?? "0.00";
      default: return '';
    }
  }
}