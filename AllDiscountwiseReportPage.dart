...
class _AllDiscountwiseReportPageState extends State<AllDiscountwiseReportPage> {
  ...
  final List<_Col> _allColumns = [
    ...
    _Col('Gross Amount', 'amount'),
    _Col('Discount Amount', 'discount'),
    _Col('Net Amount', 'netAmount'),
    _Col('Discount On Amt', 'discountOnAmt'),
  ...
  _DiscountwiseRow get totalRow {
    double sum(String Function(_DiscountwiseRow) getter) => _allRows.fold(0.0, (a, b) => a + double.tryParse(getter(b))!);

    return _DiscountwiseRow(
      restaurant: "Total",
      billNo: "",
      billDate: "",
      amount: sum((r) => r.amount).toStringAsFixed(2),
      discount: sum((r) => r.discount).toStringAsFixed(2),
      netAmount: sum((r) => r.netAmount).toStringAsFixed(2),
      discountOnAmt: sum((r) => r.discountOnAmt).toStringAsFixed(2),
      discountPercent: "", // not summed
      remark: "",
    );
  }
        ...
        body: Column(
          children: [
            ...
            Container(
              ...
              padding: const EdgeInsets.symmetric(vertical: 20, horizontal: 20), // ✅ Standard padding
              child: LayoutBuilder(
                builder: (context, constraints) {
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      // ✅ Horizontally scrollable Start & End Date inputs
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Padding(
                            padding: const EdgeInsets.only(top: 20), // ✅ Button vertical alignment
                    ...
                    itemBuilder: (context) => _allColumns.map((col) {
                      return CheckedPopupMenuItem<_Col>(
                        ...
                        child: Text(
                          col.title,
                          style: TextStyle(
                            color: col.title == 'Discount Amount' ? Colors.red[700] : Colors.black,
                            fontWeight: col.title == 'Discount Amount' ? FontWeight.bold : FontWeight.normal,
                          ),
                        ),
                      );
                    }).toList(),
...
class _DiscountwiseRow {
  ...
  factory _DiscountwiseRow.fromReport({required DiscountwiseReport report, required String restaurant}) {
    return _DiscountwiseRow(
      ...
      discountOnAmt: report.discountOnAmt,
  ...
  dynamic getField(String key) {
    switch (key) {
      ...
      case 'amount': return amount ?? "0.00";
      case 'discount': return discount ?? "0.00";
      case 'netAmount': return netAmount ?? "0.00";
      case 'discountOnAmt': return discountOnAmt ?? "0.00";
