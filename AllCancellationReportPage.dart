...
class AllCancelBillReportPage extends StatefulWidget {
  final Map<String, String> dbToBrandMap;
  const AllCancelBillReportPage({super.key, required this.dbToBrandMap});

  @override
  State<AllCancelBillReportPage> createState() => _AllCancelBillReportPageState();
}

class _AllCancelBillReportPageState extends State<AllCancelBillReportPage> {
  ...
  final List<_Col> _allColumns = [
    ...
    _Col('Cancel Date', 'cancelDate'),
    _Col('Created Time', 'createdTime'),
    _Col('Cancel Time', 'cancelTime'),
    _Col('Created User', 'createdUser'),
    _Col('Cancel User', 'cancelUser'),
    _Col('Bill Type', 'billType'),
    _Col('KOT(s)', 'kot'),
    _Col('Item(s)', 'item'),
    _Col('Taxes', 'taxes'),
  ...
  Future<void> _fetchData() async {
    ...
    Map<String, List<CancelBillReport>> dbToCancelBill =
    await UserData.fetchCancelBillForDbs(config, dbList, startDate, endDate);

    // Group by billNo
    final Map<String, List<CancelBillReport>> billGroups = {};
    for (final entry in dbToCancelBill.entries) {
      for (final report in entry.value) {
        billGroups.putIfAbsent(report.billNo, () => []).add(report);
      }
    }

    final groupedRows = <_CancelBillRow>[];
    for (final billNo in billGroups.keys) {
      final group = billGroups[billNo]!;
      final restaurant = widget.dbToBrandMap[selectedDbKey ?? "All"] ?? "ALL";
      // Merge Items (multiline), Merge KOTs (multiline), Sum taxes
      String itemsLines = "";
      String kotLines = "";
      ...
      for (final report in group) {
        // Items
        if (report.items != null && report.items.trim().isNotEmpty) {
          itemsLines += report.items
              .split(';')
              .map((e) => e.trim())
        ...
        // KOT
        if (report.kotId != null && report.kotId.trim().isNotEmpty) {
          kotLines += report.kotId + '\n';
        }
        // Taxes
        if (report.taxes != null && report.taxes.trim().isNotEmpty) {
          for (final tax in report.taxes.split(';')) {
            final parts = tax.split(':');
            if (parts.length == 2) {
              final key = parts[0].trim();
              final val =
                  double.tryParse(parts[1].replaceAll(",", "").trim()) ?? 0.0;
              taxMap[key] = (taxMap[key] ?? 0.0) + val;
            }
          }
        }
        // Other fields (from first report in group)
        billDate = report.billDate;
        cancelDate = report.cancelDate;
        createdTime = report.createdTime;
        cancelTime = report.cancelTime;
        createdUser = report.createdUser;
        cancelUser = report.cancelUser;
        billType = report.billType;
        grandTotal = report.grandTotal;
        netTotal = report.netTotal;
      }
      // Build taxes line
      String taxesLine = taxMap.entries
          .map((e) => "${e.key}: ${e.value.toStringAsFixed(2)}")
          .join("; ");
      groupedRows.add(_CancelBillRow(
        ...
        billType: billType,
        kot: kotLines.trim(),
        item: itemsLines.trim(),
        taxes: taxesLine,
        grandTotal: grandTotal,
        netTotal: netTotal,
      ));
    }
    _allRows = groupedRows;
    setState(() => _loading = false);
  }

  _CancelBillRow get totalRow {
    double sum(String Function(_CancelBillRow) getter) =>
        _allRows.fold(0.0, (a, b) =>
        a + double.tryParse(getter(b).replaceAll(',', ''))!);

    return _CancelBillRow(
      restaurant: "Total",
      billNo: "",
      billDate: "",
      cancelDate: "",
      ...
      item: "",
      taxes: "",
      grandTotal: sum((r) => r.grandTotal).toStringAsFixed(2),
      netTotal: sum((r) => r.netTotal).toStringAsFixed(2),
    );
  }
  ...
  Future<void> _exportExcel() async {
    ...
    // Report name (bold)
    final reportCell = sheet
        .cell(excel.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: rowNum));
    reportCell.value = "All Cancel Bill Report";
    ...
    final String path = '${Directory.current.path}/AllCancelBillReport.xlsx';
    ...
    return SidePanel(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Row(
            children: [
              Image.asset('assets/images/reddpos.png',
                  height: isMobile ? 28 : isTablet ? 34 : 38),
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  return Column(
                    ...
                    children: [
                      // 1st Row: Start Date + End Date with horizontal scroll
                      ...
                      // 2nd Row: Restaurants dropdown + Search button with horizontal scroll
                      SingleChildScrollView(
                        scrollDirection: Axis.horizontal,
                        child: Row(
                          crossAxisAlignment: CrossAxisAlignment.start,
...
,
                    ...
                    itemBuilder: (context) => _allColumns.map((col) {
                      return CheckedPopupMenuItem<_Col>(
                        ...
                        child: Text(
                          col.title,
                          style: TextStyle(
                            color: col.title == 'Bill No.'
                                ? Colors.red[700]
                                : Colors.black,
                            fontWeight: col.title == 'Bill No.'
                                ? FontWeight.bold
                                : FontWeight.normal,
                          ),
                        ),
                    ...
                    child: _loading
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: Column(
                            children: [
                              _buildHeaderRow(56), // Sticky header
                              _buildDataRows(48),  // Data
                              _buildTotalRow(48),  // Sticky total
                            ],
                          ),
                        ),
                      ),
  ...
  Widget _dropdownFilter(String label, List<String> dbKeys,
      String selectedDbKey, ValueChanged<String?> onChanged,
