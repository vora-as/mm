...
class AllOnlineCancelOrderWiseReportPage extends StatefulWidget {
  final Map<String, String> dbToBrandMap;
  const AllOnlineCancelOrderWiseReportPage({super.key, required this.dbToBrandMap});

  @override
  State<AllOnlineCancelOrderWiseReportPage> createState() => _AllOnlineCancelOrderWiseReportPageState();
}

class _AllOnlineCancelOrderWiseReportPageState extends State<AllOnlineCancelOrderWiseReportPage> {
  ...
  final List<_Col> _allColumns = [
    _Col('Restaurant', 'restaurantName'),
    _Col('Order Source', 'orderFrom'),
    _Col('Order No.', 'onlineOrderId'),
    _Col('Item Name', 'itemName'),
    _Col('Quantity', 'quantity'),
    _Col('Unit Price', 'unitPrice'),
    _Col('Item Gross Total', 'itemGrossTotal'),
    _Col('Order Gross Total', 'orderGrossTotal'),
    _Col('Item Total Amount', 'totalAmount'),
  ];
  late List<_Col> _visibleColumns;

  List<_OnlineCancelOrderRow> _allRows = [];
  ...
  Future<void> _fetchData() async {
    ...
    Map<String, List<OnlineCancelOrderReport>> dbToOnlineCancelOrderwise =
    await UserData.fetchOnlineCancelOrderwiseForDbs(config, dbList, startDate, endDate);

    if (selectedDbKey == null || selectedDbKey == "All") {
      for (final list in dbToOnlineCancelOrderwise.values) {
        for (final report in list) {
          _allRows.add(_OnlineCancelOrderRow.fromReport(
            report: report,
            restaurant: "ALL",
          ));
        }
  ...
  _OnlineCancelOrderRow get totalRow {
    double sum(String Function(_OnlineCancelOrderRow) getter) =>
        _allRows.fold(0.0, (a, b) => a + double.tryParse(getter(b))!);

    return _OnlineCancelOrderRow(
      restaurantName: "Total",
      orderFrom: "",
      onlineOrderId: "",
      itemName: "",
      quantity: _allRows.fold(0, (a, b) => a + (int.tryParse(b.quantity) ?? 0)).toString(),
      unitPrice: "",
      itemGrossTotal: sum((r) => r.itemGrossTotal).toStringAsFixed(2),
      orderGrossTotal: sum((r) => r.orderGrossTotal).toStringAsFixed(2),
      totalAmount: sum((r) => r.totalAmount).toStringAsFixed(2),
    );
  }
  ...
  Future<void> _exportExcel() async {
    ...
    reportCell.value = "All Online Cancel Order Wise Report";
    ...
    final String path = '${Directory.current.path}/AllOnlineCancelOrderWiseReport.xlsx';
        ...
        body: Column(
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      "All Online Cancel Order Wise Report",
                      style: TextStyle(
                        color: Colors.red,
            ...
            Container(
              ...
              padding: const EdgeInsets.symmetric(vertical: 20, horizontal: 20), // ✅ updated padding
              child: LayoutBuilder(
                builder: (context, constraints) {
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      // ✅ Start Date + End Date with horizontal scroll
                      ...
                      // ✅ Restaurants dropdown + Search button with icon and rounded shape
                      Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          _dropdownFilter(
                          ...
                          Padding(
                            padding: const EdgeInsets.only(top: 20), // ✅ consistent button alignment
...
class _OnlineCancelOrderRow {
  ...
  final String itemGrossTotal;
  final String orderGrossTotal;
  final String totalAmount;

  _OnlineCancelOrderRow({
  ...
  factory _OnlineCancelOrderRow.fromReport({required OnlineCancelOrderReport report, required String restaurant}) {
    return _OnlineCancelOrderRow(
      restaurantName: restaurant,
      orderFrom: report.orderFrom,
      onlineOrderId: report.onlineOrderId,
      itemName: report.itemName,
      quantity: report.quantity.toString(),
      unitPrice: report.unitPrice.toStringAsFixed(2),
      itemGrossTotal: report.itemGrossTotal.toStringAsFixed(2),
      orderGrossTotal: report.orderGrossTotal.toStringAsFixed(2),
      totalAmount: report.totalAmount.toStringAsFixed(2),
    );
  }
  ...
  dynamic getField(String key) {
    switch (key) {
      ...
      case 'orderFrom': return orderFrom;
      case 'onlineOrderId': return onlineOrderId;
      case 'itemName': return itemName;
      case 'quantity': return quantity;
      case 'unitPrice': return unitPrice;
      case 'itemGrossTotal': return itemGrossTotal;
      case 'orderGrossTotal': return orderGrossTotal;
