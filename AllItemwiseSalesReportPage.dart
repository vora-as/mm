...
class _AllItemwiseSalesReportPageState extends State<AllItemwiseSalesReportPage> {
  ...
  String? selectedDbKey = "All"; // Use DB key for logic
  ...
  Future<void> _fetchData() async {
    ...
    if (selectedDbKey == null || selectedDbKey == "All") {
      ...
      for (final item in allItems) {
        ...
        if (!grouped.containsKey(groupKey)) {
          grouped[groupKey] = _ItemSalesRow(
            ...
            totalQntSold: int.tryParse(item.totalQntSold) ?? 0,
            totalSaleAmount: double.tryParse(item.totalSaleAmount) ?? 0.0,
          );
        } else {
          grouped[groupKey] = grouped[groupKey]!.copyWith(
            totalQntSold: grouped[groupKey]!.totalQntSold + (int.tryParse(item.totalQntSold) ?? 0),
            totalSaleAmount: grouped[groupKey]!.totalSaleAmount + (double.tryParse(item.totalSaleAmount) ?? 0.0),
          );
        }
      }
      ...
      for (final list in dbToItemwise.values) {
  ...
  _ItemSalesRow get totalRow {
    return _ItemSalesRow(
      ...
      productCode: "",
      productName: "",
      totalQntSold: _allRows.fold(0, (a, b) => a + b.totalQntSold),
  ...
  Future<void> _exportExcel() async {
    ...
    reportCell.value = "All Itemwise Sales Report";
    ...
    final String path = '${Directory.current.path}/AllItemwiseSalesReport.xlsx';
  ...
  Widget build(BuildContext context) {
    // Build a map of {DB key: Brand Name} (already provided as widget.dbToBrandMap)
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Padding(
                            padding: const EdgeInsets.only(top: 20), // Lower the button slightly
                    ...
                    itemBuilder: (context) => _allColumns.map((col) {
                      return CheckedPopupMenuItem<_Col>(
                        ...
                        child: Text(
                          col.title,
                          style: TextStyle(
                            color: col.title == 'Product Name' ? Colors.red[700] : Colors.black,
                            fontWeight: col.title == 'Product Name' ? FontWeight.bold : FontWeight.normal,
                          ),
                        ),
                      );
                    }).toList(),
...
class _ItemSalesRow {
  ...
  dynamic getField(String key) {
    switch (key) {
      ...
      case 'totalQntSold': return totalQntSold;
      case 'totalSaleAmount': return totalSaleAmount.toStringAsFixed(2);
      default: return '';
    }
  }
